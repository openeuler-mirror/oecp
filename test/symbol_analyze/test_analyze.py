...
class ARGS(ABC):
    def __init__(self):
        ...
        self.symbol = 'unit_test'
        self.pr_id = PR_ID
        self.databse_password = ''
        self.category_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "main/data/category.json")
...
class TestRpmAnalyze(TestCase):
    def setUp(self):
        ...
        repository_old = Factory.create(old_rpm_file_path, ARGS(), "none")
        repository_new = Factory.create(new_rpm_file_path, ARGS(), "none")
        self.rpm_controller = RpmController(BRANCH_NAME, DB_PASSWORC)
        self.rpm_analyze = RpmAnalyze(ARGS(), repository_old=repository_old, repository_new=repository_new)
