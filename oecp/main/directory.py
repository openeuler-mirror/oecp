...
from .mapping import SQLiteMapping
...
class Directory(UserDict):
    ...
    def _all_focus_on_rpm(self, path):
        ...
        for path, dirs, files in os.walk(rpm_full_path):
            for file in files:
                ...
                if os.path.isfile(file_path) and RPMProxy.is_rpm_file(file_path):
                    all_rpm[file] = file_path

        if len(all_rpm) >= 2:
            for rpm, file_path in all_rpm.items():
                if RPMProxy.is_rpm_focus_on(file_path):
                    focus_rpm[rpm] = file_path
        else:
            focus_rpm = all_rpm
    ...
    def upsert_a_group(self, path, debuginfo_path=None, sqlite_path=None):
        ...
        logger.info(f"Include RPM：{','.join(sorted(focus_on_rpm.keys())[:3])}...")
...
class DistISO(Directory):
    ...
    def __init__(self, path, work_dir, category, side, with_debug=False):
        ...
        if with_debug:
            ...
            self._rpm_iso = path
        super(DistISO, self).__init__(self._rpm_iso, work_dir, category, side)
