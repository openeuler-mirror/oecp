...
from oecp.result.constants import CMP_TYPE_DIRECTORY, STAND_DISTS, CMP_RESULT_TO_BE_DETERMINED, CMP_TYPE_ISO, \
    CMP_TYPE_DIST_REPOSITORY
...
class Directory(UserDict):
    ...
    def upsert_a_group(self, path, debuginfo_path=None, sqlite_path=None):
        ...
        :return:
        """
        logger.info("%s upsert a group, path: %s, debuginfo: %s, sqlite: %s", self.verbose_path, path, debuginfo_path,
                    sqlite_path)
        ...
        logger.info("%s total %s rpm packages, %s debuginfo packages", self.verbose_path, len(focus_on_rpm),
                    len(debuginfo_rpm))
        logger.info("Include RPM：%s...", ','.join(sorted(focus_on_rpm.keys())[:3]))
        for rpm, rpm_path in focus_on_rpm.items():
            repository_name = RPMProxy.rpm_name(rpm)
...
class DistISO(Directory):
    ...
    def mount_iso(self, iso_path):
        if not iso_path or self.mounts.get(iso_path):
            return
    ...
    def umount_iso(self, iso_path):
        if not self.mounts.get(iso_path):
            return
    ...
    def set_iso_packages_sqlite_paths(self, iso):
        if not iso:
            return
        ...
        self.iso_packages_sqlite[iso] = packages2sqlite

    def compare(self, that, plan):
        try:
            self.mount_iso(self.rpm_iso)
            self.mount_iso(self.debuginfo_iso)
            self.set_iso_packages_sqlite_paths(self.rpm_iso)
            self.set_iso_packages_sqlite_paths(self.debuginfo_iso)
            ...
            packages_sqlite = self.iso_packages_sqlite.get(self.rpm_iso)
            for k, v in packages_sqlite.items():
                self.upsert_a_group(k, v, self_all_debuginfo_rpm)

            that.mount_iso(that.rpm_iso)
            that.mount_iso(that.debuginfo_iso)
            that.set_iso_packages_sqlite_paths(that.rpm_iso)
            that.set_iso_packages_sqlite_paths(that.debuginfo_iso)
            ...
            packages_sqlite = that.iso_packages_sqlite.get(that.rpm_iso)
            for k, v in packages_sqlite.items():
    ...
    def upsert_a_group(self, packages_path, sqlite=None, debuginfo_rpm=None):
...
class OEDistRepo(Directory):
    def __init__(self, paths, work_dir, category, side, arch=None):
        super(OEDistRepo, self).__init__('', work_dir, category, side)
        ...
        # paths
        # - "https://repo.openeuler.org/openEuler-20.03-LTS-SP1/EPOL/aarch64/repodata, https://repo.openeuler.org/
        # openEuler-20.03-LTS-SP1/debuginfo/aarch64/repodata"
        # - "https://repo.openeuler.org/openEuler-20.03-LTS-SP1/everything/aarch64/repodata, https://repo.openeuler.org
        # /openEuler-20.03-LTS-SP1/debuginfo/aarch64/repodata"
    ...
    def _prepare_package_info(self, package_path, debuginfo=False):
        ...
        logger.debug("%s,package_full_path=%s,sqlite_path=%s,sqlite_paths=%s", package_path, package_full_path,
                     _sqlite_path, self._sqlite_paths)
        if not _sqlite_path:
            return {}
