...
class RPMExtractDumper(AbstractDumper):
    ...
    def open_the_whitelist(self, kabi_whitelist, white_list):
        try:
            ...
            logger.debug(f"Kabi whitelist {kabi_whitelist} not exist, Please check whether the arch in "
                           f"(x86_64, aarch64), error: {err}")
    ...
    def load_white_list(self):
        ...
        if arch:
            ...
            if branch_dir:
                ...
                logger.debug(f"The branch name {white_branch} not have kabi whitelist, (Only in: 20.03-LTS-SP1, "
                               f"20.03-LTS-SP2, 20.03-LTS-SP3, 22.03-LTS, 22.03-LTS-SP1, 22.03-LTS-SP2)")
