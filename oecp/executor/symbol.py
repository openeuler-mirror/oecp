...
class SymbolCompareExecutor(CompareExecutor):
    ...
    @staticmethod
    def _extract_debuginfo_symbol(content, type):
        result = []
        cont_pattern = r'' + type + '?:\n{2}(.*?)\n{2}'
    ...
    @staticmethod
    def _extract_nodebuginfo_symbol(content, type):
        result = []
        cont_pattern = r'' + type + '? symbols? not referenced by debug info:\n{2}(.*?)\n{2}'
        all_match = re.finditer(cont_pattern, content, flags=re.S)
        for match in all_match:
            if match:
                for line in match.group(1).split('\n'):
    ...
    def _extract_changed_abi(self, string_content):
        ...
        for type in SYMBOL_TYPE:
            ...
            elif type == CHANGE_VAR:
                one_type_result = self._extract_deubginfo_changed_var(string_content)
            else:
                one_type_debuginfo_result = self._extract_debuginfo_symbol(string_content, type)
                one_type_nodebuginfo_result = self._extract_nodebuginfo_symbol(string_content, type)
                one_type_result = one_type_debuginfo_result + one_type_nodebuginfo_result
            if one_type_result:
                symbol_result.setdefault(type, one_type_result)
    ...
    def collect_effect_rpm_by_symbol(self, symbol_result, file):
        ...
        for type, symbols in symbol_result.items():
            if 'Added' in type or not symbols:
                continue
            for symbol in symbols:
                filter = {'u_symbol_table': symbol, 'association_so_name': f'"{elf_name}"'}
                query_results = self.rpm_controller.query_symbol_contains_so(filter)
                logger.debug(f"Query {elf_name}->{symbol} effected record. ")
                if query_results:
                    effect_details, ef_rpm = self._upsert_query_result(elf_name, query_results, symbol)
                    effect_rpms.extend(ef_rpm)
                    so_file_result.setdefault(type, []).extend(effect_details)
    ...
    def _count_abi_change_result(symbol_result):
        count_result = {}
        for type, result in symbol_result.items():
            if result:
                count_result.setdefault(type, len(result))
        return count_result
    ...
    def _compare_result(self, dump_a, dump_b, single_result=CMP_RESULT_SAME):
        ...
        for pair in common_pairs:
            ...
            out_result = self.judge_abi_out_details(out)
            if err:
            ...
            if not out_result:
                ...
                if symbol_result:
                    logger.info(f"Extract {base_b} changed abi: {symbol_result}")
                all_effect_info, effect_rpm = self.collect_effect_rpm_by_symbol(symbol_result, pair[0])
                if effect_rpm:
                    logger.debug(f"{rpm_b}: {base_b} effect rpms:{effect_rpm}")
        ...
        if remove_files:
            ...
            for removed_file in remove_files:
                ...
                result.set_cmp_result(CMP_RESULT_DIFF)
                result.add_component(data)
            logger.debug(f"{rpm_b} remove files contains: {removed_file_name}, and effect rpms: {all_effect_rpm}")
