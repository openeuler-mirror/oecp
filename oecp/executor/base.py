...
class CompareExecutor(ABC):
    ...
    @staticmethod
    def judge_abi_out_details(out):
        changed_num = False
        if out:
            pattern_summary = r"(Function|Variable)(.*)changes summary:"
            pattern_calculate = r"(\d+) (Removed|Changed|Added)"
            for line in out.split('\n')[:4]:
                if not re.match(pattern_summary, line):
                    continue
                calculate_group = re.findall(pattern_calculate, line)
                for single_group in calculate_group:
                    if int(single_group[0]) == 0:
                        continue
                    changed_num = True
                    break
    ...
    @staticmethod
    def component_new_symbol(datas):
        all_pvds = [' '.join([pvd['name'], pvd['symbol'], pvd['version'].split('-')[0]]) for pvd in datas]

        return set(all_pvds)

    @staticmethod
    def pretty_symbol_datas(datas, flag_vrd):
        # 获取require、provide名称与多版本映射关系
        pretty_result = {}
    ...
    def format_dump_provides(self, base_datas, other_datas, flag_vrd):
        ...
        prov_base = self.pretty_symbol_datas(base_datas, flag_vrd[0])
        prov_other = self.pretty_symbol_datas(other_datas, flag_vrd[1])
        ...
        all_components_a = self.component_new_symbol(base_datas)
        all_components_b = self.component_new_symbol(other_datas)
    ...
    def format_dump_requires(self, base_datas, other_datas, flag_vrd):
        """
        比较两个rpm软件包的require差异
        @param base_datas: 基准rpm的全量requires信息
        @param other_datas: 待比rpm的全量requires信息
        @param flag_vrd: 软件包的version、release、dist信息
        @return: 差异结果 dict
        """
        common_files, base_common_files, other_common_files = [], [], []
        diff_requires = []
        req_base = self.pretty_symbol_datas(base_datas, flag_vrd[0])
        req_other = self.pretty_symbol_datas(other_datas, flag_vrd[1])
        for base_side in req_base.keys():
            if req_other.get(base_side):
                base_requires = sorted(set(req_base.get(base_side)))
                other_requires = sorted(set(req_other.get(base_side)))
                if base_requires == other_requires:
                    common_files.append([','.join(base_requires), ','.join(other_requires), CMP_RESULT_SAME])
                else:
                    diff_requires.append([','.join(base_requires), ','.join(other_requires), CMP_RESULT_DIFF])
                base_common_files.extend(base_requires)
                other_common_files.extend(other_requires)

        all_components_base = self.component_new_symbol(base_datas)
        all_components_other = self.component_new_symbol(other_datas)
        less_requires = all_components_base - set(base_common_files)
        more_requires = all_components_other - set(other_common_files)

        all_dump = [
            common_files,
            diff_requires,
            [[x, '', CMP_RESULT_LESS] for x in less_requires],
            [['', x, CMP_RESULT_MORE] for x in more_requires]
        ]
