...
class LibCompareExecutor(CompareExecutor):
    ...
    def _compare_result(self, dump_base, dump_other, single_result=CMP_RESULT_SAME):
        ...
        if not base_libs and not other_libs:
            logger.debug(
                f"No {self.config.get('compare_type')} package found, ignored with {dump_base['rpm']} and {dump_other['rpm']}")
            return result
    ...
    def compare(self):
        ...
        for single_pair in similar_dumpers:
            if single_pair:
                ...
                result = self._compare_result(dump_base, dump_other)
