...
from oecp.result.constants import CMP_RESULT_SAME, CMP_TYPE_RPM, CMP_RESULT_DIFF, CMP_TYPE_SERVICE_DETAIL, \
    CMP_RESULT_LESS, CMP_RESULT_MORE, CMP_SERVICE_SAME, CMP_TYPE_RPM_SERVICE
...
class ServiceCompareExecutor(CompareExecutor):
    ...
    def _compare_result(self, dump_a, dump_b, single_result=CMP_RESULT_SAME):
        ...
        for pair in common_file_pairs:
            ...
            data = CompareResultComponent(CMP_TYPE_RPM_SERVICE, file_result, base_a, base_b)
            result.add_component(data)
        ...
        if only_file_a:
            for file_a in only_file_a:
                ...
                data = CompareResultComponent(CMP_TYPE_RPM_SERVICE, CMP_RESULT_LESS, side_a, '')
                result.add_component(data)
                count_result["less_count"] += 1
        if only_file_b:
            for file_b in only_file_b:
                side_b = self._intercept_file_name(file_b, 'half')
                data = CompareResultComponent(CMP_TYPE_RPM_SERVICE, CMP_RESULT_MORE, '', side_b)
                result.add_component(data)
