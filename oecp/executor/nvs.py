...
from oecp.result.constants import CMP_RESULT_CHANGE, CMP_TYPE_PROVIDES, CMP_TYPE_KABI, CMP_TYPE_KCONFIG, \
    CMP_TYPE_REQUIRES
...
class NVSCompareExecutor(CompareExecutor):
    ...
    def _to_pretty_dump(self, dump):
        ...
        pretty_dump.setdefault(rpm_n, [])
        for component in dump[self.data]:
            ...
            pretty_dump.get(rpm_n, []).append(new_component)
        return rpm_n, pretty_dump

    def _cmp_component_set(self, dump_a, dump_b, components_a, components_b):
        """
        ...
        if dump_a['kind'] == CMP_TYPE_KABI or dump_a['kind'] == CMP_TYPE_KCONFIG:
            ...
            component_results = self.format_dump_requires(components_a, components_b, flag_v_r_d)
    ...
    def compare(self):
        ...
        for dump_a in self.dump_a:
            for dump_b in self.dump_b:
                # 取rpm name 相同进行比较
                if RPMProxy.rpm_name(dump_a['rpm']) == RPMProxy.rpm_name(dump_b['rpm']) or dump_a['kind'] == 'kabi' or \
                        dump_a['kind'] == 'kconfig':
                    if dump_a['kind'] in [CMP_TYPE_PROVIDES, CMP_TYPE_REQUIRES]:
                        components_a = dump_a[self.data]
                        components_b = dump_b[self.data]
                    else:
