...
class KoCompareExecutor(CompareExecutor):
    ...
    def __init__(self, base_dump, other_dump, config=None):
        ...
        self.data = 'data'
        self._whitelist_rpms = self.get_whitelist_rpms()
    ...
    def get_whitelist_rpms():
        whitelist_rpms_path = os.path.realpath(
            os.path.join(os.path.dirname(__file__), '../conf/ko_whitelist_rpms/whitelist_rpms.yaml'))
        with open(whitelist_rpms_path, "r") as f:
            whitelist = yaml.safe_load(f)

        return whitelist

    def _set_ko_info_details(self, result, component_dump, white_list, src_name, cmp_result=CMP_RESULT_SAME):
        for component_result in component_dump:
            for sub_component_result in component_result:
                ...
                if cmp_result == CMP_RESULT_SAME:
                    ...
                    elif belong_white_list == "No" and src_name in self._whitelist_rpms:
                        cmp_result = CMP_RESULT_DIFF
                        result.set_cmp_result(cmp_result)
    ...
    def _compare_result(self, base_dump, other_dump, cmp_result=CMP_RESULT_SAME):
        ...
        if not base_map_ko and not other_map_ko:
            logger.debug(f"No {kind} modules found!")
            return result
        ...
        for pair in common_ko_pairs:
            ...
            detail_result = CompareResultComposite(CMP_TYPE_KO_INFO, CMP_RESULT_SAME, base_ko, other_ko)
            cmp_result = self._set_ko_info_details(detail_result, component_dump, kabi_white_list, src_name)
