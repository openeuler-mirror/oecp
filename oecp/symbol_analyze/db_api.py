...
class Api(object):
    ...
    def get_session(self, database_name, db_password):
        ...
        try:
            engine = create_engine(data_url, connect_args={'connect_timeout': 30}, pool_recycle=3600)
            ...
            raise ValueError("connect database error, ip=%s, port=%s, database=%s, detail=%s", ip, port, database_name,
                             e) from e
        else:
            return db_session
    ...
    def bulk_save_objects(self, object_list):
        try:
            ...
            raise ValueError("bulk save objects error!,detail: %s", e) from e
        else:
            self.session.commit()
    ...
    def select_symbol_contains_so(self, filters=None, limit=None):
        ...
        if filters.get('u_symbol_table', None):
            query = query.filter(Symbol.u_symbol_table == filters.get('u_symbol_table'))
        if filters.get('association_so_name', None):
            query = query.filter(Symbol.association_so_name.contains(filters.get('association_so_name')))
        if limit is not None:
            query = query.limit(limit)
    ...
    def create_symbols(self, context, values):
        ...
        try:
            ...
            raise ValueError("An exception occurred when adding symbols: '%s'", e) from e
        return service_ref
    ...
    def query_count(self, model, filters=None):
        ...
        if filters.get('rpm_name', None):
            query = query.filter(model.rpm_name == filters.get('rpm_name'))
        count = query.scalar()
    ...
    def delete_by_rpm_name(self, model, rpm_name):
        try:
            ...
            self.session.rollback()
            raise ValueError("delete by rpm name error, mode:%s, rpm_name:%s, detail:%s", model, rpm_name, e) from e
    ...
    def delete_pr_mapping_by_pr_id(self, pr_id):
        try:
            ...
            self.session.rollback()
            raise ValueError("delete pr mapping by pr id error, pr_id:%s, detail=%s", pr_id, e) from e
