...
from oecp.result.constants import ALL_ARCH
...
class RpmAnalyze(object):
    ...
    def get_work_dir(self, work_dir):
    ...
    def submit_symbol_change(self):
        for arch in ALL_ARCH:
            database_name = '-'.join([self.branch_name, arch])
    ...
    def construct_database(self, is_tmp_table=False):
        rpm_controller = RpmController(self.branch_name, self.password)
        if is_tmp_table:
            ...
            try:
                ...
                pr_mappings = rpm_controller.query_pr_mappings_by_pr_id(self.pr_id)
            logger.info(
                f"Total query {len(pr_mappings)} pr_mapping result from database {self.branch_name} table pull_request")
            for pr in pr_mappings:
                if pr.is_rpm_new:
                    rpm_controller.bulk_delete_datas(SymbolTemporary, pr.rpm_name)
            rpm_controller.delete_pr_mapping_by_pr_id(self.pr_id)
        ...
        for rpm_name, repository in self.repository_new.items():
            ...
            if rpm_path:
                ...
                if all_files:
                    ...
                    logger.debug('%s does not have binary or so.', rpm)
    ...
    def construct_symbol_structures(self, rpm_controller, binary_files, rpm_name, model):
        ...
        logger.debug('Number of %s construct symbol data: %s', rpm_name, data_number)
    ...
    def collect_symbols(self, so_file):
        ...
        logger.debug("%s has %s out symbols.", so_file.split('__rpm__')[-1], len(out_symbols))
        return out_symbols
