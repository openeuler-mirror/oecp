...
def parse_case_summary(log_summary):
    res = re.findall(r'A total of (\d+) use cases were executed, with (\d+) successes and (\d+) failures.', log_summary)
    if not res:
        return
    print("osv_smoke_nr_total:", res[0][0], file=sys.stdout)
    print("osv_smoke_nr_pass:", res[0][1], file=sys.stdout)
    print("osv_smoke_nr_fail:", res[0][2], file=sys.stdout)
    ...
    with open(result_path) as f:
        for line in f:
            ...
            if CASE_END_TEXT in line:
                log_text[2] = line
                case_name = parse_case_name(log_text)
                if case_name:
                    result[case_name] = 1
                log_text = ['', '', '']
    return result
