...
def group_rpm_name_result(detail, single_result, side_a, side_b, rows, base_side_a, base_side_b):
    ...
    if result in RESULT_DIFF:
        assign_diff_details(detail, pkg_name, single_result, base_side_a, base_side_b, side_a, side_b, rows.get(pkg_name))
        ...
        if is_same(rpm_result):
            ...
            assign_diff_details(detail, pkg_name, single_result, base_side_a, base_side_b, side_a, side_b, rpm_result)

def assign_diff_details(detail, pkg_name, single_result, base_side_a, base_side_b, side_a, side_b, rpm_result):
    ...
    for cmp_type, results in rpm_result.items():
        if cmp_type == CMP_TYPE_RPM_FILES:
            ...
            for single_result in results:
                ...
                if result == "same":
                    continue
                obj = single_result.get(base_side_a) if single_result.get(base_side_a) else single_result.get(base_side_b)
