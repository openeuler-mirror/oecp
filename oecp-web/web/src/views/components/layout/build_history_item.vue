<template>
    <div>
        <div class="history" style="width: 320px; height: 370px;">
            <a-carousel :dots="false" ref="carousel_build">
                <div style="height:340px" v-for="(item_data, index) in his_data" :key="index">
                    ...
                    <a-steps progress-dot direction="vertical" size="small" :current="item_data.step" :status="(item_data.status=='ERROR' ?'error':'finish')">
                        ...
                        <a-step title="解析报告" disabled>
                            <template v-if="item_data.step == 3 && item_data.status=='ERROR'">
                                <div slot="description">{{item_data.error_msg}}</div>
                            </template>
                            <template v-else>
                        ...
                        <a-step title="完成" disabled description="完成iso上传、解析、持久化"/>
                    </a-steps>
...
export default {
    ...
    methods: {
        page_change(page){
            this.$refs.carousel_build.goTo(page, false)
        }
    }
}
</script>
