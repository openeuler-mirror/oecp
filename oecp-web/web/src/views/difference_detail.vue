    ...
    <div class="list_page_container">
        <mx-search-box>
            ...
            <div slot="searchbox">
                ...
                <mx-search-box-item title="compare result">
                    <a-select v-model="search.compare_result" style="width:200px">
                        <a-select-option key="all" value="">all</a-select-option>
        ...
        <mx-table-page :pageIndex="pageIndex" :pageSize="pageSize" :total="total" @on-page-change="changepage">
            <a-table :data-source="tables" :columns="columns" rowKey="id" :pagination="false" :expandRowByClick="true" :loading="loading" @change="handleChange">
                <span slot="source_title" class="form-table-heard">
                    {{info.source}}
                </span>
                <span slot="target_title" class="form-table-heard">
                    {{info.target}}
                </span>
                <template slot="link_path" slot-scope="text, row">
...
export default {
    ...
    data () {
        return {
            ...
            columns: [
                ...
                {key: 'source', dataIndex: 'source',slots: {title:"source_title"}, sorter: true},
                {key: 'compare', dataIndex: 'compare', slots: {title:"target_title"}, sorter: true},
                ...
                {title: 'effect drivers', key: 'effect_drivers', dataIndex: 'effect_drivers', sorter: true},
                {title: 'details path', key: 'detail_path', scopedSlots: {customRender:"link_path"}, sorter: true},
            ],
            tables: [],
    ...
    async mounted() {
        ...
        self.search.report_base_id = this.$route.query.report_base_id
        self.$http.get(`report/compare/all-compare-type/${self.search.report_base_id}?key=different`).then(res=>{
            self.compare_types = res.data.data
        })
        self.$http.get(`report/compare/all-compare-result/${self.search.report_base_id}?key=different`).then(res=>{
            self.compare_results = res.data.data
        })
        let res = await self.$http.get(`report/compare/iso-info/${self.search.report_base_id}`)
    ...
    methods: {
        ...
        getData () {
            var self = this
            self.loading = true
            self.$http.get(`report/detail/all-differences/page/${self.pageSize}/${self.pageIndex}`, {
