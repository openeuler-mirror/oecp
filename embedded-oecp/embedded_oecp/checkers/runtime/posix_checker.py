...
class PosixChecker(BaseChecker):
    ...
    def run(self) -> List[TestResult]:
        ...
        results = []
        results.append(self._check_posix())
        return results
    ...
    def _prepare_posix_config(self, mugen_dir: str, sdk_path: str) -> str:
        logger = get_logger()
        template_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
            "assets", "embedded_osv_posix.json",
        )

        if not os.path.isfile(template_path):
            logger.error(f"POSIX config template not found: {template_path}")
            return ""
        ...
        config_path = os.path.join(mugen_dir, "combination", "embedded_osv_posix.json")
        os.makedirs(os.path.dirname(config_path), exist_ok=True)

        with open(config_path, "w", encoding="utf-8") as f:
            json.dump(config, f, indent=4, ensure_ascii=False)

        logger.info(f"POSIX config prepared: {config_path}")
        return config_path

    def _run_posix_tests(self, mugen_dir: str, config_path: str) -> Tuple[str, int, str, str]:
        logger = get_logger()
        ...
        try:
            ...
            logger.error(f"Failed to start POSIX tests: {e}")
            return cmd, -1, "", str(e)
    ...
    def _parse_combination_results(self, mugen_dir: str) -> Tuple[int, int, List[str]]:
        ...
        for combo_dir_name in os.listdir(results_dir):
            ...
            if not os.path.isdir(combo_dir):
                continue
            if "posix" not in combo_dir_name.lower():
                continue
    ...
    def _save_execute_log(self, stdout: str, stderr: str, rc: int) -> str:
        log_path = os.path.join(self._evidence_dir, "posix_execute_log.txt")
        with open(log_path, "w", encoding="utf-8") as f:
            f.write("=" * 60 + "\n")
            f.write("POSIX 测试执行日志\n")
            f.write("=" * 60 + "\n\n")
            f.write(f"退出码: {rc}\n\n")
            f.write("--- stdout ---\n")
    ...
    def _check_posix(self) -> TestResult:
        logger = get_logger()
        result = TestResult(
            category="运行时", sub_item="POSIX",
            requirement="使用 mugen 执行 POSIX 测试用例",
            acceptance_criteria="POSIX 测试用例结果与社区一致",
        )
        ...
        config_path = self._prepare_posix_config(mugen_dir, sdk_path)
        if not config_path:
            result.fail_test(detail="无法生成 POSIX 测试配置文件")
            return result

        cmd, rc, stdout, stderr = self._run_posix_tests(mugen_dir, config_path)

        log_path = self._save_execute_log(stdout, stderr, rc)
        logger.info(f"POSIX execute log saved: {log_path}")

        exec_lines = [
            {"text": "# POSIX 测试执行", "color": "#6c7086"},
        ...
        path_exec = os.path.join(self._evidence_dir, "posix_step1_execute.png")
        render_terminal_screenshot("Step 1: POSIX 测试执行", exec_lines, path_exec)

        total, passed, failed_cases = self._parse_combination_results(mugen_dir)

        if total == 0:
            result.fail_test(
                detail="POSIX 测试执行完成但未解析到测试结果",
                evidence=f"rc={rc}, screenshot={path_exec}, log={log_path}",
                remediation="请检查 mugen 版本和 POSIX 测试配置",
            )
            return result
        ...
        result_lines = [
            {"text": "# POSIX 测试结果汇总", "color": "#6c7086"},
            {"text": "", "color": "#cdd6f4"},
        ...
        if failed_cases:
            ...
            result_lines.append({"text": f"所有 {total} 个 POSIX 用例全部通过", "type": "success"})

        result_lines.append({"text": "", "color": "#cdd6f4"})
        if passed == total:
            result_lines.append({"text": f"POSIX 测试全部通过: {passed}/{total}", "type": "success"})
        else:
            result_lines.append({"text": f"POSIX 测试未全部通过: {passed}/{total}", "type": "error"})

        path_result = os.path.join(self._evidence_dir, "posix_step2_results.png")
        render_terminal_screenshot("Step 2: POSIX 测试结果", result_lines, path_result)
        ...
        if passed == total:
            ...
            result.pass_test(
                detail=f"所有 POSIX 测试用例通过: {passed}/{total}",
                evidence=evidence,
            )
        else:
            ...
            result.fail_test(
                detail=f"POSIX 测试未全部通过: {passed}/{total}（失败: {failed_detail}）",
                evidence=fail_evidence,
                remediation="请检查 glibc 及内核兼容性",
            )
